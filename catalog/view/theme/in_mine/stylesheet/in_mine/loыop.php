<?php
	...
	elseif($portfolio_layout == 'Half Thumbnails Without Spaces'){
		$col='st_sf_col col-md-6 st_sf_x2';
	}
	elseif($portfolio_layout == 'Full Thumbnails Without Spaces'){
		$col='st_sf_col col-md-12 st_sf_x2';
	}
    elseif($portfolio_layout == 'Full Thumbnails List'){
		$col='st_sf_col col-md-12 st_sf_x2';
	}
	?>
    <div style="padding:0px!important;" class="st_sf_strange_portfolio_item st_sf_port_style_ii <?php echo esc_attr($col);?> <?php echo esc_attr($slugg)?>">
        <div style="padding: 5px; height:100%;">
		<div class="st_sf_vc_potrfolio"  style="background:url('<?php echo esc_url($large_image_url[0]); ?>');padding:0!important;">		
           <?php if($portfolio_layout == 'Full Thumbnails Without Spaces'){ ?>
            <div class="oi_creative_p_content">
                <div class="arrow-up"></div>
                <img class="prize_img" width="48" src="<?php echo get_template_directory_uri().'/framework/img/prize.png'; ?>">
                <div class="oi_creative_p_holder">
                    <h3 class="oi_c_title"><a href="<?php the_permalink($post->ID)?>"><?php the_title();?></a></h3>
                    <span class="oi_c_cats"><?php echo esc_attr(substr($slug, '0', '-2'));?></span>
                </div>
            </div>
            <?php }else{ ?>

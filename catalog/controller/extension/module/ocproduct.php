...
class ControllerExtensionModuleOcProduct extends Controller {
	public function index($setting) {
		...
		if($setting['option'] == 0) {
			...
			if($setting['productfrom']==1){
				...
				if ($setting['input_specific_product']==0){
                    ...
                    $data['products_new_total'] = $this->model_catalog_product->getTotalProducts($filter_total);
                    $data['products_new_current'] = count($results);
                    $data['products_new_load_link'] = $this->url->link('extension/module/ocproduct/additional');
                    $data['products_new_settings'] = $setting;
		...
		if ($results) {
			...
			foreach ($results2 as $res) {
				$data['images'][] = array(
					'product_id' => $result['product_id'],
					'popup' => $this->model_tool_image->resize($res['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height')),
					'thumb' => $this->model_tool_image->resize($res['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height'))
				);
			}
	...
	public function additional() {
	    ...
	    if (isset($request['cate_id']) && $request['limit'] && $request['width'] && $request['height'] && $request['offset']) {
            $this->load->model('catalog/product');
            $this->load->model('extension/module/ocproduct');
            $this->load->model('tool/image');
            ...
            $filter_data = array(
                'filter_category_id' => $request['cate_id'],
                'sort' => 'p.date_added',
                'order' => 'DESC',
            ...
            if ($results) {
                foreach ($results as $result) {
                    if ($result['image']) {
                        $image = $this->model_tool_image->resize($result['image'], $request['width'], $request['height']);
                    } else {
                        $image = $this->model_tool_image->resize('placeholder.png', $request['width'], $request['height']);
                    }

                    $c_words = 50;
                    $name = strlen($result['name']) > $c_words ? substr($result['name'],0,$c_words)."..." : $result['name'];
